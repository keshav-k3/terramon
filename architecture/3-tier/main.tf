# 3-Tier Architecture
# Web tier (public) -> Application tier (private) -> Database tier (private)
...
# Web Tier - Application Load Balancer Security Group
...
resource "aws_launch_template" "web" {
  ...
  user_data = base64encode(<<-EOF
              ...
              echo "<h1>3-Tier Architecture - Web Server $(hostname -f)</h1>" > /var/www/html/index.html
              echo "<p>Application Backend: ${aws_lb.app.dns_name}</p>" >> /var/www/html/index.html
              EOF
  )
...
resource "aws_launch_template" "app" {
  ...
  user_data = base64encode(<<-EOF
              #!/bin/bash
              yum update -y
              yum install -y java-11-amazon-corretto mysql

              # Create simple health check endpoint
              mkdir -p /opt/app
              cat > /opt/app/health.html <<'EOH'
              OK
              EOH
              ...
              echo "Database Endpoint: ${aws_db_instance.main.endpoint}" > /opt/app/db-config.txt
              EOF
  )
